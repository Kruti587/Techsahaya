...
class ChatService:
    ...
    def answer(self, message: str, language: str = "en", profile: EligibilityProfile | None = None) -> ChatResponse:
        logger.info("--- CHAT ARCHITECTURE PIPELINE STARTED ---")
        # 1. USER QUERY
        logger.info("[STEP 1] USER QUERY Received")
        self.schemes = load_schemes()
        self.scheme_map = {s.id: s for s in self.schemes}
        self.rules = load_rules()
        ...
        # PII Protection Check (Step 1.1)
        if self._contains_pii(cleaned_message):
            logger.warning("PII detected in user query.")
            return ChatResponse(
                answer="Sensitive identity number detected. Please do not share your Aadhaar or PAN numbers here.",
                schemes=[],
                evidence=[],
                verification_status="blocked_by_security_gateway",
                confidence="low",
                offline_ready=True,
            )

        # AI Security Gateway Pre-check (Step 1.2)
        if settings.prompt_injection_guard_enabled and self._is_adversarial_or_jailbreak(cleaned_message):
            ...
            return ChatResponse(
                answer="This request attempts to access restricted system information. Therefore, Request blocked.",
                schemes=[],
                evidence=[],
                verification_status="blocked_by_security_gateway",
                confidence="low",
        ...
        logger.info("[STEP 2] Intent Detection: '%s'", intent)

        # 3. Citizen Context Extraction
        logger.info("[STEP 3] Citizen Context Extraction: Profile provided = %s", profile is not None)
        eval_profile = profile or EligibilityProfile()

        # 4. Relevant Scheme Retrieval
        logger.info("[STEP 4] Relevant Scheme Retrieval")
        ...
        # 5. Eligibility Rule Evaluation
        logger.info("[STEP 5] Eligibility Rule Evaluation")
        ...
        logger.info("[STEP 6] Alternative Scheme Search")
        if eligibility_result and eligibility_result.status == "not_eligible" and matched_schemes:
            primary_scheme = matched_schemes[0]
            alt_schemes = [self.scheme_map[sid] for sid in primary_scheme.alternative_scheme_ids if sid in self.scheme_map]
            if alt_schemes:
                logger.info("Found %d alternative schemes.", len(alt_schemes))
                # Add alternatives to matched schemes for LLM context
                for alt in alt_schemes:
                    if alt not in matched_schemes:
                        matched_schemes.append(alt)

        # 7. Evidence Validation
        logger.info("[STEP 7] Evidence Validation")
        evidence = [
            {
        ...
        logger.info("[STEP 8] Confidence Calculation")
        confidence = self._calculate_confidence(chunks, matched_schemes, cleaned_message)

        # 9. LLM Explanation
        logger.info("[STEP 9] LLM Explanation Generation")
        ...
        logger.info("[STEP 10] Action Recommendation (Tour/Action)")
        ...
        logger.info("--- CHAT ARCHITECTURE PIPELINE COMPLETED ---")
    ...
    def _contains_pii(self, text: str) -> bool:
        """Detect Aadhaar or PAN numbers to prevent PII leakage."""
        if not text:
            return False
        ...
        if re.search(aadhaar_pattern, text_upper) or re.search(pan_pattern, text_upper):
            return True
        return False
