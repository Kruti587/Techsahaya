...
async def upload_document(request: Request, file: UploadFile = File(...), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    document, extracted_attributes, newly_eligible_schemes = document_service.process_upload(db, user, file, content)
    profile = profile_service.get_or_create(db, user)
    ...
    if document.document_type not in existing_documents:
        ...
        db.add(profile)
        db.commit()
    audit_service.log(db, "document_uploaded", f"{document.document_type} uploaded via OCR flow", user.id, get_user_role(db, user.id), f"document:{document.id}", request)
    return {
        "status": "processed",
        ...
        "newly_eligible_schemes": newly_eligible_schemes,
        "message": "Processed in memory via OCR and discarded. Only masked metadata is retained.",
    }
