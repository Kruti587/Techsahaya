...
export function SchemeDetailsPage() {
  ...
  const handleCheckEligibility = async () => {
    if (isOfflineMode || !navigator.onLine) {
      // TRUE OFFLINE EDGE COMPUTING DEMO
      const { evaluateOfflineEligibility } = await import("../utils/offlineEngine");
      
      // Mocked cached rule for demo purposes
      const cachedRule = {
        age: schemeId === "sukanya-samriddhi" ? { min: 0, max: 10 } : null,
        gender: schemeId === "sukanya-samriddhi" ? "Female" : "Any",
        income: schemeId === "pmay-g" ? { max_annual: 200000 } : null,
        state_scope: ["All"],
        occupation_required: schemeId === "pm-kisan" ? ["Farmer"] : [],
      };
