    ...
    <div className="space-y-6">
      <section className="rounded-3xl bg-white p-6 shadow-card" data-tour="welfare-gaps-header">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <Link to="/profile" className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-sahaya-green px-4 font-semibold text-white">
            {t(language, "completeProfile")} <ArrowRight size={18} />
          </Link>
          <Link to="/documents" className="inline-flex min-h-12 items-center gap-2 rounded-xl border px-4 font-semibold text-sahaya-green">
        ...
        <div className="space-y-4" data-tour="welfare-gaps-list">
          ...
          {!loading && gaps.map((gap) => (
            <article key={gap.scheme} className="rounded-2xl border p-4">
              ...
              <Link to="/eligibility" className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-sahaya-green px-4 text-sm font-semibold text-white">
                {t(language, "reviewEligibility")} <ArrowRight size={16} />
              </Link>
            </article>
          ))}
        </div>
      ...
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm text-sahaya-green">
        ...
        {language === "hi"
          ? "छूटे हुए लाभ की जांच केवल आपकी प्रोफ़ाइल से होती है। इसके लिए आधार या पैन की आवश्यकता नहीं है।"
          : language === "kn"
          ? "ತಪ್ಪಿಸಿಕೊಳ್ಳಬಹುದಾದ ಪ್ರಯೋಜನಗಳ ಪತ್ತೆ ನಿಮ್ಮ ಪ್ರೊಫೈಲ್‌ನಿಂದ ಮಾತ್ರ ನಡೆಯುತ್ತದೆ. ಆಧಾರ್ ಅಥವಾ ಪ್ಯಾನ್ ಅಗತ್ಯವಿಲ್ಲ."
          : "Gap detection uses your own profile only. It does not require Aadhaar or PAN for basic checks."}
      </div>
    </div>
  );
}
